...
% Suppression des méta-données de conférence pour le rendu "propre"
\settopmatter{printacmref=false}
\setcopyright{none}
\renewcommand\footnotetextcopyrightpermission[1]{}
\pagestyle{plain}

% --- Fichier de Bibliographie intégré (pour faciliter la compilation) ---
...
@article{vorhees2021trec,
  ...
  url={https://dl.acm.org/doi/10.1145/3451964.3451965},
...
\begin{itemize}
    ...
    \item \textbf{Qrels:} Expert relevance judgments used as Ground Truth, with relevance scores on a 0-2 scale (0: not relevant, 1: partially relevant, 2: highly relevant).
\end{itemize}
...
\subsubsection{Evaluation Framework Benefits}
Our reusable evaluation framework (\texttt{functions.py}) provided several advantages:
\begin{itemize}
    \item \textbf{Consistency:} All models evaluated with identical search and scoring logic
    \item \textbf{Reproducibility:} Standardized evaluation ensures fair comparison
    \item \textbf{Efficiency:} Pre-computed document vectors enable rapid experimentation
\end{itemize}

\subsubsection{Computational Considerations}
\begin{itemize}
    \item \textbf{TF-IDF:} Fastest inference, sparse representations enable efficient search
    \item \textbf{Word2Vec:} Moderate speed, requires tokenization and averaging
    \item \textbf{DAN:} Slowest due to neural forward passes, but enables learning
    \item \textbf{Transformers:} Moderate speed with batch processing, but requires significant memory for 171K documents
\end{itemize}
