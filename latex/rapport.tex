...
% Allow more flexible hyphenation to avoid overflow
\sloppy
...
Figure~\ref{fig:vocab_mismatch} illustrates this vocabulary mismatch problem and how semantic models address it.
...
\caption{Vocabulary Mismatch Problem: Traditional exact-match systems (TF-IDF) fail when query terms differ from document terms, even when they share the same meaning. Semantic models capture semantic equivalence through embeddings.}
\label{fig:vocab_mismatch}
\end{figure}
...
The complete source code, notebooks, and experimental results are available at: \url{https://github.com/Lounes-KEBDI/medical_information_retrieval.git}

\section{Related Work}

\subsection{TREC-COVID Challenge and Previous Approaches}
The TREC-COVID challenge \cite{roberts2021searching} was established in response to the urgent need for effective information retrieval systems during the COVID-19 pandemic. Conducted over five rounds from April to July 2020, this shared task attracted participation from 92 unique teams and generated 556 individual submissions. The challenge aimed to construct a pandemic search test collection and evaluate IR methods specifically for COVID-19 scientific literature. The dataset consists of 50 topics (queries) representing real-world information needs, with topics added progressively to target emerging research questions as the pandemic evolved. This comprehensive evaluation framework has become a standard benchmark for biomedical information retrieval systems.

Previous work on TREC-COVID has explored various approaches. Top-performing systems in the challenge typically combined multiple techniques: lexical methods (BM25), neural reranking with BERT-based models, query expansion, and hybrid architectures. Many successful submissions leveraged domain-specific pre-trained models (BioBERT, PubMedBERT) with task-specific fine-tuning on retrieval pairs. However, most studies focused on achieving high performance through ensemble methods and complex pipelines, with limited analysis of why individual approaches succeed or fail. Few works have provided systematic comparisons across the full spectrum from simple baselines (TF-IDF) to state-of-the-art transformers, or investigated the trade-offs between model complexity and data requirements in this low-resource setting.

\subsection{Evolution of Information Retrieval Methods}
...
The introduction of Word Embeddings, such as Word2Vec \cite{mikolov2013efficient}, allowed for vector representations where similar words are close in space. These static embeddings capture semantic relationships but lack contextual understanding. In the medical domain, models pre-trained on biomedical corpora, such as BioBERT \cite{lee2020biobert}, have shown that domain adaptation is crucial for performance. However, BioBERT inherits the vocabulary of general BERT, causing medical terms to be fragmented into subword tokens. PubMedBERT \cite{gu2020domain} addresses this by training from scratch on biomedical corpora, constructing a domain-specific vocabulary that avoids term fragmentation and achieves superior performance on biomedical benchmarks.
...
More recently, Reimers et al. introduced Sentence-BERT \cite{reimers2019sentence}, optimizing BERT architectures specifically for semantic similarity tasks using siamese networks. This approach addresses a critical limitation of standard BERT: the [CLS] token embedding is not optimized for cosine similarity in dense retrieval scenarios. Subsequent work has extended this paradigm to biomedical domains, with models like MedCPT \cite{jin2023medcpt} leveraging contrastive learning on large-scale search logs to align query and document embeddings. Hybrid approaches combining lexical (BM25) and semantic (dense) signals, such as SPLADE \cite{formal2021splade}, have also shown promise by learning sparse lexical representations that maintain interpretability while benefiting from contextual understanding.

\subsection{Our Contributions}
This work addresses several gaps in previous TREC-COVID studies. First, we provide a \textbf{systematic evaluation framework} using reusable, model-agnostic functions that ensure fair comparison across all approaches—from simple TF-IDF to complex transformers. This framework eliminates implementation biases and allows us to isolate the impact of model architecture choices.

Second, we conduct a \textbf{comprehensive comparative analysis} spanning the full spectrum of IR methods: lexical baselines (TF-IDF), static embeddings (Word2Vec variants), neural architectures (DAN), and transformer models (BioBERT, Sentence-BERT). Unlike previous work that focused on ensemble methods, we evaluate each approach independently to understand their individual strengths and limitations.

Third, we provide \textbf{detailed failure analysis} explaining why certain theoretically promising approaches (e.g., BioBERT, fine-tuned models) underperform despite their advantages. Our analysis reveals critical insights: (1) domain expertise alone is insufficient without task-specific optimization, (2) model complexity requires adequate training data, and (3) simple approaches can outperform complex architectures in low-resource scenarios.

Finally, we investigate the \textbf{trade-offs between model complexity and data availability} in specialized domains. Our experiments demonstrate that trainable models (DAN, fine-tuned Word2Vec) suffer from overfitting with limited queries (50), while pre-trained models achieve better generalization. These findings provide practical guidance for IR system design in resource-constrained biomedical settings.
...
\begin{itemize}
    ...
    \item \textbf{Qrels:} Expert relevance judgments used as Ground Truth, with relevance scores on a -1--2 scale (-1: Document not evaluated, 0: not relevant, 1: partially relevant, 2: highly relevant).
\end{itemize}
...
Figure~\ref{fig:ir_pipeline} illustrates the general information retrieval pipeline used across all models.

\begin{figure}[h]
\centering
\begin{tikzpicture}[node distance=1.5cm, auto]
    % Query
    \node[rectangle, draw, fill=blue!20, text width=2.5cm, align=center] (query) {User Query\\"loss of smell"};
    
    % Corpus
    \node[rectangle, draw, fill=gray!20, text width=2.5cm, align=center, right=3cm of query] (corpus) {Corpus\\171,332 docs};
    
    % Vectorization
    \node[rectangle, draw, fill=orange!20, text width=2.5cm, align=center, below=1.5cm of query] (query_vec) {Query\\Vector};
    \node[rectangle, draw, fill=orange!20, text width=2.5cm, align=center, below=1.5cm of corpus] (doc_vecs) {Document\\Vectors};
    
    % Similarity
    \node[circle, draw, fill=green!20, text width=2cm, align=center, below=1.5cm of query_vec] (similarity) {Cosine\\Similarity};
    
    % Ranking
    \node[rectangle, draw, fill=purple!20, text width=2.5cm, align=center, below=1.5cm of similarity] (ranking) {Top-K\\Ranking};
    
    % Results
    \node[rectangle, draw, fill=red!20, text width=2.5cm, align=center, below=1.5cm of ranking] (results) {Retrieved\\Documents};
    
    % Arrows
    \draw[->, thick] (query) -- (query_vec);
    \draw[->, thick] (corpus) -- (doc_vecs);
    \draw[->, thick] (query_vec) -- (similarity);
    \draw[->, thick] (doc_vecs) -- (similarity);
    \draw[->, thick] (similarity) -- (ranking);
    \draw[->, thick] (ranking) -- (results);
    
    % Model annotation
    \node[above=0.3cm of query_vec, text width=3cm, align=center] {\small \textbf{Model-specific}\\
    TF-IDF / Word2Vec /\\
    DAN / BERT / etc.};
    
    % Evaluation
    \node[rectangle, draw, fill=yellow!20, text width=2.5cm, align=center, right=2cm of results] (eval) {Evaluation\\NDCG@10};
    \draw[->, thick, dashed] (results) -- (eval);
    \node[rectangle, draw, fill=gray!30, text width=2.5cm, align=center, above=1cm of eval] (qrels) {Qrels\\Ground Truth};
    \draw[->, thick, dashed] (qrels) -- (eval);
\end{tikzpicture}
\caption{Information Retrieval Pipeline: All models follow the same pipeline: (1) vectorize query and documents using model-specific methods, (2) compute cosine similarity, (3) rank documents, (4) retrieve top-K results, and (5) evaluate against expert judgments (Qrels).}
\label{fig:ir_pipeline}
\end{figure}
...
\textbf{1. W2V-General (Trained from Scratch):} We trained a Word2Vec model from scratch using the entire TREC-COVID corpus. \textbf{Training data:} All 171,332 documents from the corpus were used. For each document, we combined the title and text, tokenized using NLTK's \texttt{word\_tokenize}, removed stopwords, and filtered to keep only alphabetic tokens. This produced a training corpus of 171,332 tokenized documents with an average length of 94 words per document. \textbf{Technical implementation:} We used Gensim's Word2Vec with the following parameters: 200-dimensional embeddings, window size 5 (context window of 5 words on each side), minimum word count 2 (words appearing less than twice are ignored), and trained for 5 epochs (5 complete passes over the corpus). The model learned embeddings for 105,771 unique words present in the TREC-COVID corpus. \textbf{Important note:} This model is trained unsupervised on the document corpus only; the 50 test queries are not used during training and are only used for evaluation.

\textbf{2. BioWord2Vec (Pre-trained):} We used a pre-trained model (BioWordVec\_PubMed\_MIMICIII\_d200) trained on 1.5 million biomedical words from PubMed and MIMIC-III. This model provides domain-specific embeddings without requiring any training on our corpus. It was used directly for inference without modification.
\textbf{3. BioWord2Vec Fine-tuned:} We fine-tuned the pre-trained BioWord2Vec model on the TREC-COVID corpus. \textbf{Training data:} All 171,332 documents from the TREC-COVID corpus were used, processed identically to the W2V-General model (title + text, tokenization, stopword removal). \textbf{Technical implementation:} We created a new Word2Vec model with the same architecture as the pre-trained model (200 dimensions, window size 5). We then built a vocabulary from the TREC-COVID documents, resulting in 81,225 unique words. For words present in both vocabularies (69,868 common words), we initialized their embeddings with the pre-trained weights, preserving the biomedical knowledge learned from PubMed/MIMIC-III. For words unique to TREC-COVID (11,357 new words), we initialized embeddings randomly and learned them during fine-tuning. The model was then trained for 5 epochs on the TREC-COVID documents, allowing it to adapt the embeddings to corpus-specific terminology while retaining general biomedical knowledge. \textbf{Important note:} As with W2V-General, only the document corpus is used for training; queries are not used during fine-tuning.

\textbf{Document Representation:} For all Word2Vec variants, documents are represented by averaging word embeddings. We combine title and text, tokenize using NLTK, remove stopwords, and compute the mean embedding vector. Queries are processed identically during inference (not during training).
...
Figure~\ref{fig:dan_architecture} illustrates the DAN architecture and encoding process.

\begin{figure}[h]
\centering
\begin{tikzpicture}[node distance=1.2cm, auto]
    % Input text
    \node[rectangle, draw, fill=blue!20, text width=2.5cm, align=center] (input) {Text\\"loss of smell"};
    
    % Tokenization
    \node[rectangle, draw, fill=yellow!20, text width=2.5cm, align=center, below=0.8cm of input] (token) {Tokenization\\``loss'', ``smell''};
    
    % Embeddings
    \node[rectangle, draw, fill=orange!20, text width=2.5cm, align=center, below=0.8cm of token] (embed) {BioWord2Vec\\Embeddings\\200d each};
    
    % Average
    \node[circle, draw, fill=purple!20, text width=2cm, align=center, below=0.8cm of embed] (avg) {Average\\200d};
    
    % MLP
    \node[rectangle, draw, fill=green!20, text width=3cm, align=center, below=0.8cm of avg] (mlp) {MLP\\
    Linear(200→300)\\
    ReLU + Dropout\\
    Linear(300→200)};
    
    % Output
    \node[rectangle, draw, fill=red!20, text width=2.5cm, align=center, below=0.8cm of mlp] (output) {Output\\Embedding\\200d};
    ...
    % Bi-encoder annotation
    \node[right=2cm of mlp, text width=3cm, align=left] {\small \textbf{Bi-Encoder:}\\
    Same encoder for\\
    queries \& documents\\
    (shared weights)};
    
    % Dimensions annotation
    \node[left=0.3cm of embed] {\small 200d};
    \node[left=0.3cm of avg] {\small 200d};
    \node[right=0.3cm of mlp] {\small 300d};
    \node[left=0.3cm of output] {\small 200d};
\end{tikzpicture}
\caption{DAN Architecture: The encoder processes text through tokenization, BioWord2Vec embeddings, averaging, and a Multi-Layer Perceptron (MLP) to produce optimized 200-dimensional embeddings. The same encoder is used for both queries and documents (bi-encoder architecture).}
\label{fig:dan_architecture}
\end{figure}
